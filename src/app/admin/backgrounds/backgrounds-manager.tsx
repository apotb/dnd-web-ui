...
import { upsertBackgroundEntry, deleteBackgroundEntry } from "@/lib/content/catalog";
...
export function BackgroundsManager({ entries }: { entries: ContentEntry[] }) {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      ...
      const result = await upsertBackgroundEntry(slug, form.name.trim(), form.source.trim() || "Custom", data);
      if (result.error) { setFormError(result.error); return; }
      const row: ContentEntry = { slug, name: form.name.trim(), source: form.source.trim() || "Custom", extra: data as Record<string, unknown> };
      setList((prev) => {
  ...
  function handleDelete(row: ContentEntry) {
    ...
    startTransition(async () => {
      const result = await deleteBackgroundEntry(row.slug);
    ...
    <div className="space-y-4">
      <div className="retro-box">
        <div className="flex gap-2 flex-wrap items-center">
          ...
          <Button onClick={openCreate}>+ Add Background</Button>
        </div>
        <p className="text-xs text-muted-foreground mt-2">{filtered.length} of {list.length} backgrounds</p>
      </div>
      ...
      <div className="retro-box" style={{ padding: 0 }}>
        ...
        {filtered.length === 0 && <p className="text-sm text-muted-foreground text-center py-8">No backgrounds found. Click "Seed from PHB" to populate.</p>}
      ...
      <Dialog open={dialogOpen} onOpenChange={(o) => { if (!o) setDialogOpen(false); }}>
        <DialogContent className="w-full sm:max-w-[720px] max-h-[90vh] overflow-y-auto p-8">
          <DialogHeader className="mb-2">
            <DialogTitle className="text-xl">{editRow ? `Edit: ${editRow.name}` : "New Background"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-5">
            ...
            <div className="space-y-1.5">
              <Label>Data (JSON — PhbBackground shape)</Label>
              <p className="text-xs text-muted-foreground">{`{ "skillProficiencies": ["insight","religion"], "languageChoices": 2, "equipment": ["belt pouch"], "gold": 15, "feature": { "name": "...", "description": "..." } }`}</p>
