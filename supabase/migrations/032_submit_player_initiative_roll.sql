-- Allow players to submit initiative rolls without DM/service-role access.
...
BEGIN
  ...
  IF v_all_done AND jsonb_array_length(v_tokens) > 0 THEN
    SELECT COALESCE(
      jsonb_agg(token_id ORDER BY total DESC, dex_mod DESC, label ASC),
      '[]'::jsonb
    )
    INTO v_order
    FROM (
...
REVOKE ALL ON FUNCTION public.submit_player_initiative_roll(UUID, UUID, INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.submit_player_initiative_roll(UUID, UUID, INTEGER) TO authenticated;
