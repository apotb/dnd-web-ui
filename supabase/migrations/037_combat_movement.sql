-- Combat movement: sync token moves and per-turn movement budget.
...
GRANT EXECUTE ON FUNCTION public.apply_combat_move(UUID, TEXT, INTEGER, INTEGER, INTEGER, BOOLEAN) TO authenticated;

-- Reset movement budget when advancing turns.
...
-- Backfill movement fields on existing turn state.
UPDATE public.campaigns
SET combat_state = jsonb_set(
  jsonb_set(
    combat_state,
    '{turn,movementUsedFeet}',
    COALESCE(combat_state #> '{turn,movementUsedFeet}', '0'::jsonb),
    TRUE
  ),
  '{turn,dashUsed}',
  COALESCE(combat_state #> '{turn,dashUsed}', 'false'::jsonb),
  TRUE
)
