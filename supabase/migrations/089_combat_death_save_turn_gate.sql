-- Block ending turn until the active party combatant rolls a death save when dying.
...
BEGIN
  ...
  IF COALESCE(v_token->>'kind', '') = 'party' AND v_character IS NOT NULL THEN
    v_character_combat := COALESCE(v_character.data->'combat', '{}'::jsonb);
    IF COALESCE((v_character_combat->>'currentHp')::INT, 1) = 0
      AND COALESCE(v_character_combat->'conditions', '[]'::jsonb) @> '["dying"]'::jsonb
      AND COALESCE((v_turn->>'deathSaveRolled')::BOOLEAN, FALSE) = FALSE THEN
      RAISE EXCEPTION 'Make your death saving throw before ending this turn.';
    END IF;
  END IF;
